...
impl Parser {
    ...
    fn parse_prefix(&mut self) -> Result<Expression, ErrorsEmitted> {
        ...
        match token {
            Some(
                ...
                //    if let Ok(Token::DblDot { .. } | Token::DotDotEquals { .. }) =
    ...
    fn parse_infix(&mut self, left_expr: Expression) -> Result<Expression, ErrorsEmitted> {
        ...
        match token {
            ...
            Some(Token::LParen { .. }) => {
                let call_expr = CallExpr::parse(self, left_expr)?;

                Ok(Expression::Call(call_expr))
            }

            Some(Token::LBracket { .. }) => {
                let index_expr = IndexExpr::parse(self, left_expr)?;
