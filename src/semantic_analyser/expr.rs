...
pub(crate) fn analyse_expr(
    ...
    match expression {
            ...
            match symbol {
                Some(
                    ...
                    if Type::UserDefined(path.clone()) == receiver_type {
                        let method_path = path.push(mc.method_name.to_type_path());
            ...
            let path = match p.tree_opt.clone() {
                Some(mut segments) => {
                    if let Some(id) = segments.pop() {
                        ...
                        match &p.path_root {
                            PathRoot::Identifier(i) => root.push(i.to_type_path()),
        ...
        Expression::Struct(s) => {
            let type_path = root.push(TypePath::from(s.struct_path.clone()));
        ...
        Expression::TupleStruct(ts) => {
            let type_path = root.push(TypePath::from(ts.struct_path.clone()));
